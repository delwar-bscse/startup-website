...
const FinancialInvestmentDetails: React.FC<any> = ({
  onHandleStep,
  user,
  refetch,
}) => {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      projectDes: user?.projectDes || "",
      haveCompany: user?.haveCompany || "",
      haveTeam: user?.haveTeam || "",
      ifHaveCompany: user?.ifHaveCompany || "",
      experience: user?.experience || "",
      shareSell: user?.shareSell || "",
      needCapital: user?.needCapital || "",
      percentageSell: user?.percentageSell || "",
    },
  });
  ...
  useEffect(() => {
    if (user) {
      form.reset({
        projectName: user?.financialDetails?.annualIncome || "",
        projectDes: user?.financialDetails?.debtToIncomeRatio || "",
        haveCompany: user?.financialDetails?.cashAndSavings || "",
        haveTeam: user?.financialDetails?.investmentHorizon || "",
        ifHaveCompany: user?.financialDetails?.investmentGoal || "",
        experience: user?.financialDetails?.riskTolerance || "",
        shareSell: user?.financialDetails?.experience || "",
        needCapital: user?.financialDetails?.previousInvestment || "",
        percentageSell: user?.financialDetails?.returnOnInvestment || "",
      });
    }
  }, [user, form]);
