            ...
            {stepsData.map((step) => (
              <div key={step.id} className="flex items-start gap-4">
                <div className="flex flex-col items-center gap-2 pt-[2px]">
                  <p className={`w-6 h-6 bg-gray-300 rounded-full flex ${activeStep === step.id ? 'bg-primary2' : 'bg-gray-300'}`} />
                  {stepsData.length !== step.id && <div className="grow border-s-2 h-8 border-dashed border-gray-300" />}
                </div>
                <h2 className={`text-2xl leading-6 ${activeStep === step.id ? 'text-primary2' : 'text-gray-500'}`}>{step.title}</h2>
              </div>
            ))}
          </div>
        </div>
